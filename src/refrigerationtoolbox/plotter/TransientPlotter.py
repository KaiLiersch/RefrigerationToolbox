...
class TransientPlotter:
    ...
    def plot_temperatures(self, ax : Axes, transient : TransientCycle, t_scale : float = 3600.0,
                          t_label : str = "Time $t$ (h)", celsius : bool = True,
                          lw : float = 2.0, markersize : float = 5) -> Axes:
        ...
        ax.plot(t, transient.Tc - offset, "-o", color=RED, lw=lw, markersize=markersize,
                label="Condensation $T_c$", zorder=4)
        ax.plot(t, transient.Te - offset, "-o", color=BLUE, lw=lw, markersize=markersize,
                label="Evaporation $T_e$", zorder=4)
        self._mark_infeasible(ax, t, np.concatenate([transient.Te, transient.Tc]) - offset,
    ...
    def plot_cop(self, ax : Axes, transient : TransientCycle, t_scale : float = 3600.0,
                 t_label : str = "Time $t$ (h)", cooling : bool = True,
                 lw : float = 2.0, markersize : float = 5) -> Axes:
        """Plot the coefficient of performance over time, heating and optionally cooling."""
        t = self._time(transient, t_scale)

        ax.plot(t, transient.COP_heat, "-o", color=GREEN, lw=lw, markersize=markersize,
                label="Heating $COP_{heat}$", zorder=4)
        values = [transient.COP_heat]
        if cooling:
            ax.plot(t, transient.COP_cool, "--o", color=YELLOW, lw=lw, markersize=markersize,
                    label="Cooling $COP_{cool}$", zorder=4)
            values.append(transient.COP_cool)
        self._mark_infeasible(ax, t, np.concatenate(values),
                              np.concatenate([transient.feas] * len(values)))
    ...
    def plot_transient(self, axes : Sequence[Axes] | np.ndarray, transient : TransientCycle,
                       t_scale : float = 3600.0,
                       t_label : str = "Time $t$ (h)") -> Sequence[Axes] | np.ndarray:
    ...
    @staticmethod
    def _mark_infeasible(ax : Axes, t : np.ndarray, values : np.ndarray, feas : np.ndarray,
                         color : str = "black", markersize : float = 9) -> Axes:
        """Cross out the points in time at which the heat exchangers violate their constraints."""
        if np.all(feas):
            return ax
        t_all = np.tile(t, values.size // t.size)
        ax.plot(t_all[~feas], values[~feas], "x", color=color, markersize=markersize,
                label="Infeasible", linestyle="none", zorder=6)
        return ax
